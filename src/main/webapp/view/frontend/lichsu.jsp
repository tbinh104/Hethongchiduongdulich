	...
	<div class="main">
		...
		<div class="content">
			...
			<div class="location">
				...
				<div class="place">
				<c:forEach var="Lichsu" items="${Lichsu}">
					<div class="place__card but"  onclick="ModalClick(),MapClick(${Lichsu.diadiemdulich.vitri.getVtkinhdo()},${Lichsu.diadiemdulich.vitri.getVtvido()}),SubmitFR(${Lichsu.diadiemdulich.getDddl_id()})">						
						<div class = "img_wraper" style='background-image: url("${pageContext.request.contextPath}${Lichsu.diadiemdulich.getDddl_path()}")'>				
						</div>
						<div class="place__content gioihanchu">						
							<h3>${Lichsu.diadiemdulich.getDddl_ten()}</h3>
							<h4>${Lichsu.getLsthoigian()}</h4>
							<h5>${Lichsu.diadiemdulich.mien.getTenmien()}</h5>
							<p>${Lichsu.diadiemdulich.getDddl_mota()}</p>														
						</div>			
					</div>
					<form:form id="${Lichsu.diadiemdulich.getDddl_id()}" 		
						method="post"
						action="/lichsu"
						modelAttribute="Lichsu1">
						<form:input type="hidden" 
							value="${Lichsu.diadiemdulich.getDddl_id() }"
							path="diadiemdulich"
							name="dddlid"
						/><br/>
						<form:input type="hidden"
							value="${Lichsu.diadiemdulich.getDddl_ten() }"
							path="diadiemdulich"
							name="dddlten"
						/><br/>
						<form:input type="hidden" 
							value="${Lichsu.diadiemdulich.getDddl_mota() }"
							path="diadiemdulich"
							name="dddlmota"
						/><br/>
						<form:input type="hidden"
